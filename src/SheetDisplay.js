...
function GetSheet() {
  ...
  if (currentDataSheet == null) {return;}
...
function GetMonth(NameOfMonth) {
  ...
  MonthIndex.forEach(function(Value) {
    if (Value.Name != NameOfMonth) {return}
...
function GetDayOfTheWeek(DayByNumberOrString) {
  ...
  DayIndex.forEach(function(Value){
    if (DayByNumberOrString == Value.Day)  {indexToReturn = Value}
    if (DayByNumberOrString == Value.Name) {indexToReturn = Value}
  })
...
function CacheImportantData() {
  ...
  for (;;) {
    ...
    if (DonationSheetCache.StartingCellPositionsInA1.length > 33) {break;}
...
function AdvanceToPosition(Position) {
  ...
  if (PositionToReturn != null) { print(`Advanced to: ${Position}`) }
...
function CreateCellPositions() {
  ...
  for (var i = 0;  i < AmountOfCellsToMake; i++) {
    ...
    NewPosition.range.startRowIndex = DonationCellMeta.CellIndexOrigin.startRowIndex + CurrentOffset;
    NewPosition.range.endRowIndex =  DonationCellMeta.CellIndexOrigin.endRowIndex + CurrentOffset;
    NewPosition.range.startColumnIndex =  DonationCellMeta.CellIndexOrigin.startColumnIndex;
    NewPosition.range.endColumnIndex =  DonationCellMeta.CellIndexOrigin.endColumnIndex;

    NewPosition.rows = [{
      values: [{
        userEnteredValue: {stringValue: `${i + 1}`}
      }]

    }]

    NewPosition.fields = "userEnteredValue"
